...
/*
$Log: T49Run.C,v $
Revision 1.29  2008/01/04 10:48:59  cblume
64bit modifications by Marek Szuba

Revision 1.28  2007/11/05 18:30:07  cblume
T49EventRoot.C
...
Bool_t T49Run::Open(Char_t *FileName,Int_t MaxEvents)
{
  ...
  if (fRootFile && fRootFile->IsOpen()) {
    ...
    if (fVerbose) {
      printf("T49Run::Open(): Found fTree at %p\n",fTree);
    ...
    if (fTree) {
      ...
      //fTree->SetBasketSize("*",64000); //yeni deyishdim//8000 idi
      ////printf("test333333\n");
...
//______________________________________________________________________________
void T49Run::SetEvetoCal(Int_t tdcal)
{
...
void T49Run::CheckEvetoCal()
{
  ...
  if ((fAutoInit) &&
    ...
    if (fTdCal==1) {
      ...
      InitEvetoCalibration(1,1468);//Calibration procedure of A. Laszlo for 158A GeV
//      printf("veto init 1468\n");

    }
    else if (fTdCal==3){
      InitEvetoCalibration(1,3162);//Calibration procedure of A. Laszlo for 40A GeV
//      printf("veto init 3162\n");
...
//   printf("actual run: %i, reference run: %i\n",fRunNumber,fTdRefRun);
  ...
  if (fTdRefRun >= 0) {
    ...
    if (fin3) {
      ...
      fin3.getline(dumb,1000);

      while (fin3 >> runv >> av >> ave >> bv >> bve) {


// 	      printf("reading run %i\n",runv);
...
//           printf("reference run2: %i\n",fTdRefRun);


      if (fTdRefRun==1468) { //reference run for Andras procedure for 158A GeV
	    fTdAref  = -0.00743806;
            fTdBref  = 276.75;
            refFound = 1;
      ...
      } else if (fTdRefRun==3162) { //reference run for Andras procedure for 40A GeV
	    fTdAref  = -0.0171263;
            fTdBref  = 126.879;
            refFound = 1;
...
//            printf("reference run2.7: %i\n",fTdRefRun);
  ...
  if (centOn) {
    ...
    if (fin2) {
      ...
      fin2 >> dumb >> fXsec;
//       printf("reference run2.75: %i\n",fTdRefRun);

      while (fin2 >> cent >> evetocut) {
//       printf("reference runx: %i\n",fTdRefRun);
...
//	printf("cent: %4.2f, evetocut: %8.1f\n",cent,evetocut);

      }

      fin2.close();
//       printf("reference run2.8: %i\n",fTdRefRun);
