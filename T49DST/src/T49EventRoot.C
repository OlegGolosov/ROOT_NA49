...
Revision 1.21  2007/11/05 18:30:16  cblume
Update by Banjamin
...
//	    printf("i: %i, eveto: %8.0f, upEveto: %8.0f, upCentr: %4.2f\n",i,eveto,gT49->GetCentrBins(i),gT49->GetCentrBinX(i));
      i++;
    }
...
//      lowCentr = (i-1.)/2.+0.5;
    ...
    if (i > 0) {
      lowEveto = gT49->GetCentrBins(i-1);
       ...
       lowEveto=2*gT49->GetCentrBins(0)-gT49->GetCentrBins(1);
    }
...
//    printf("upCentr: %4.2f, lowCentr: %4.2f, centr: %4.2f\n",upCentr,lowCentr,centr);
...
//    cout << "upCentr: " << upCentr << " lowCentr: " << lowCentr << " centr: " << centr << endl;
...
Int_t T49EventRoot::GetCentralityClass2()
{

	Int_t refrun=gT49->GetTDCalRefRun();
	Float_t  fEvetoCut[5];

if (refrun==1468) {

	fEvetoCut[0]=0.280331*158.;
  	fEvetoCut[1]=0.452036*158.;
  	fEvetoCut[2]=0.658987*158.;
  	fEvetoCut[3]=0.79941*158.;
  	fEvetoCut[4]=0.901069*158.;
// 	printf("ref run: %i\n",gT49->GetTDCalRefRun());

} else if (refrun==3162) {

	fEvetoCut[0]=0.159087*40.;
  	fEvetoCut[1]=0.301091*40.;
  	fEvetoCut[2]=0.494815*40.;
  	fEvetoCut[3]=0.646027*40.;
  	fEvetoCut[4]=0.765062*40.;
// 	printf("ref run: %i\n",gT49->GetTDCalRefRun());

} else {
		printf("wrong reference run (should be 1468 for 158A GeV, 3162 for 40A GeV)\n");
		printf("initialize with run->SetEvetoCal(2) for 158A GeV and run->SetEvetoCal(3) for 40A GeV !!!\n");

		return 0;
	}

	Float_t eveto=GetTDCalEveto();
	Int_t i=0;

	  while ((eveto > fEvetoCut[i]*207.) &&
           (i < 5)) {
      i++;
    }
    i++;
