...
typedef struct event_mc_ext_t {   /* MC event header extension */  
  struct point_tpc_mc_t   *pnt_gpc_p;   /* Pointer to first point in VT1 */  /* ??FZ */
  struct point_tpc_mc_t    *pdr_gpc_p[7];    /* First point on padrow */     /* ??FZ */
  struct track_mc_ext_t    *track_ext_p;
} event_mc_ext_t;
...
typedef struct track_mc_t {   /* Monte Carlo track structure */
   ...
   struct point_tgt_mc_t *pnt_tgt_p;        /* Pnter to point on target surface */
...
/************************************************************************/
typedef struct track_mc_ext_t {   /* Monte Carlo track structure extension */
   Int_t                    n_gpc_hit;        /* Nhits in vertex tpc2 */        /* ??FZ */
   struct point_tpc_mc_t *pnt_gpc_p;        /* 1st l.l. pnt of track VTPC1 */   /* ??FZ */
   struct ref_tpc_mc_t   *ref_gpc_p;  /* Pnter to trk param. at ref. surf. */   /* ??FZ */
} track_mc_ext_t;

/************************************************************************/
typedef struct beam_mc_t {   /* Monte Carlo beam structure */
   Int_t                    pid;              /* Particle ID (GEANT codes) */
   Int_t                    use_random;       /* Simulation of beam divergence and offsets */
   Float_t                  div_x;            /* Beam divergence in x */
   Float_t                  div_y;            /* Beam divergence in y */
   Float_t                  offset_x;         /* Beam offset in x */
   Float_t                  offset_y;         /* Beam offset in y */
   struct beam_track_mc_t   *track_p;         /* Pnter to beam track */
} beam_mc_t;

/************************************************************************/

typedef struct beam_track_mc_t {  /* Monte Carlo beam_track structure */
   Float_t                  px;               /* px momentum */
   Float_t                  py;               /* py momentum */
   Float_t                  pz;               /* pz momentum */
   Float_t                  ptot;             /* Total momentum */
   Float_t                  x;                /* x position at generation point */
   Float_t                  y;                /* y position at generation point */
   Float_t                  z;                /* z position at generation point */
   struct vertex_mc_t       *vertex_p;        /* Pnter to interaction vertex */
   struct track_mc_t        *daughter_p;      /* Pnter to first daughter */
} beam_track_mc_t;

/************************************************************************/

typedef struct target_mc_t {     /* Monte Carlo target structure */
   Int_t                    medium;          /* Target medium */
   Float_t                  rho;             /* Target density in g/cm3 */
   Float_t                  lambda;          /* Target nuclear interaction length in g/cm2 */
   Float_t                  dz;              /* Target length in cm */
   Float_t                  x;               /* Target center x position */
   Float_t                  y;               /* Target center y position */
   Float_t                  z;               /* Target center z position */
   Float_t                  tilt_x;          /* Target tilt around x (rad) */
   Float_t                  tilt_y;          /* Target tilt around y (rad) */
} target_mc_t;
...
typedef struct point_tgt_mc_t {   /* Monte Carlo target point structure */
   Float_t                 x;                /* x */
   Float_t                 y;                /* y */
   Float_t                 z;                /* z */
   Float_t                 px;               /* px momentum */
