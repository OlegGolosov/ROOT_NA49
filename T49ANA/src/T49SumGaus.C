...
T49SumGaus::T49SumGaus(Char_t* name): TF1()
{
 //TF1(name,(Double_t (*)(Double_t *, Double_t *)) 0,0.5,2.0,9)
  ...
  SetName(name);
  fNpar = 12;        // Was 9 for symmetric peaks
  fXmin      = 0.5;
  fXmax      = 2.0;
  fNpx       = 100;
  fType      = 0;
  ...
  SetParNames("PiRes","ElRelPos","PiPos","KaRelPos","PRelPos","DRelPos","ElAmp","PiAmp","KaAmp","PAmp");
  SetParName(10,"DAmp");
  SetParName(11,"Asymm");
  SetParLimits(1,1,-1);
  SetParLimits(3,1,-1);
...
{
  if (!fNpHist) {
    std::cerr << "T49SumGaus: You have to set the number-of-point histogram!!" << std::endl;
    return 0;
  }
  if (!params)
    params=fParams;
  ...
  for (Int_t i=0; i<fNPeaks;i++) {
	...
	par2[2]=params[0]*pow(fabs(params[i+1]),fPowSigScale);
    }
    par2[0]=params[i+fNPeaks+1];
    par2[3]=params[11];
    f+=OnePeak(x[0],par2);
  }
  return f;
...
void T49SumGaus::SetFree()
{
  ...
  if (!fAmpFix[4] || GetAmplitude(4) > 0) SetParLimits(5,0,0);
}
...
void T49SumGaus::InitPositions(Float_t pipos)
{
  ...
  SetPosition(1,pipos);
  for (Int_t i=0;i<fNPeaks;i++) if (i!=1) SetParameter(1+i,GetRelRise(i)/GetRelRise(1));
  SetParLimits(1,1,-1);
  SetParLimits(3,1,-1);
  SetParLimits(4,1,-1);
  SetParLimits(5,1,-1);
...
void T49SumGaus::SetAmplitude(Int_t i, Float_t x,Float_t lowlim, Float_t uplim)
{
  ...
  SetParameter(fNPeaks+1+i,x);
  SetParLimits(fNPeaks+1+i,lowlim,uplim);
}
...
void T49SumGaus::SetResolution(Float_t x)
{
  if (GetParameter(2)!=0) {
    if (fLinSigScale)
      ...
      SetParameter(0,x*pow(fabs(GetParameter(2)),fPowSigScale));
  }
  else {
...
{
  return GetParameter(fNPeaks+1+i);
}
...
Float_t T49SumGaus::GetResolution()
{
  if (fLinSigScale)
    ...
    return GetParameter(0)/pow(GetParameter(2),fPowSigScale);
}
...
Float_t T49SumGaus::GetMaximum(Int_t i)
{
  ...
  if (i==1) {
    ...
    if (fLinSigScale)
      ...
      par2[2]=fParams[0]*pow(fabs(fParams[i+1]),fPowSigScale);
  }
  par2[0]=fParams[i+fNPeaks+1];
  par2[3]=fParams[9];
...
Float_t T49SumGaus::GetAsymmetry() {
  return Float_t(GetParameter(11));
}
...
void T49SumGaus::StoreCovMat()
{
  ...
  std::cerr << "WARNING: StoreCovMat does not work correctly anymore" << std::endl;
  std::cerr << "Need to propagate parameter renumbering" << std::endl;
  Double_t *pCovMat=(Double_t *) fCovMatMinuit;
  //TVirtualFitter *gFitter=TVirtualFitter::Fitter(0,25);
  gMinuit->mnemat(pCovMat,12);
  if (gMinuit->GetNumFixedPars()!=0) {
  //Int_t k=0;
    Double_t val,err;
    // Remap matrix, taking care of fixed parameters
    for (Int_t i=11;i>=0;i--) {
      Int_t k=gMinuit->GetParameter(i,val,err);
      //Float_t err1=gFitter->GetParError(i);
      //Int_t l=0;
      for (Int_t j=11;j>=0;j--) {
	Int_t l=gMinuit->GetParameter(j,val,err);
	//Float_t err2=gFitter->GetParError(j);
	//if (err1==0||err2==0) 
	if (k==0||l==0) 
	  fCovMatMinuit[i][j]=0;
	else {
	  //fCovMatMinuit[i][j]=gFitter->GetCovarianceMatrixElement(k,l);
...
void T49SumGaus::RestoreParameters()
{
  ...
  if (fStored) {
	...
	fAmpFix[i-fNPeaks-1]=1;
      else
	fAmpFix[i-fNPeaks-1]=0;
      }
    }
  }
...
void T49SumGaus::DrawPeaks(Int_t LineWidth)
{
  ...
  Double_t par[fNPeaks]; 
  if (!fGraphs)
    fGraphs=new TObjArray(fNPeaks);
  for (Int_t p=0; p<fNPeaks; p++) {
    if (p==1) {
      par[1]=fParams[2];
      par[2]=fParams[0];
	...
	par[2]=fParams[0]*pow(fabs(fParams[p+1]),fPowSigScale);
    }
    par[0]=fParams[p+fNPeaks+1];
    par[3]=fParams[11];
...
// Find maxima
  ...
  */
  // Mod 16 feb 2003
  Float_t MaxAmp=Hist->GetEntries()*Hist->GetBinWidth(1);
  InitPositions(PiPos);
...
//  ** Code provided by Christof Roland **
  ...
  Double_t mass[5] = {MASS_E_2, MASS_PI_2, MASS_K_2, MASS_P_2, MASS_D_2};
