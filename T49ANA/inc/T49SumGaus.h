...
public:
  ...
  TH1*      GetNpHist() { return fNpHist; }

  Float_t   RelRise(Float_t Ptot, Int_t Part);
  Float_t   GetRelRise(Int_t part);
  Float_t   GetStep() {return fXstep;}
  TGraph   *GetPeak(Int_t i) { if (fGraphs) return (TGraph*)fGraphs->At(i); else return 0; }

  Double_t  fCovMatMinuit[12][12];
  Float_t   fCovMat[12][12];
  ...
  Float_t  fPtot;
  Bool_t   fAmpFix[5];
  Double_t fStoredPars[12];
  Double_t fStoredLowLim[12];
  Double_t fStoredUpLim[12];
  Bool_t   fStored;
